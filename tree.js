class Tree {
  ...
  render(treeNodes) {
    ...
    this.elem.dispatchEvent(new CustomEvent("fleet-rendered"));
  }
  ...
  buildTree(nodes, depth) {
    ...
    if(nodes && nodes.length) {
      nodes.forEach(node => {
        const liEle = Tree.createLiEle.call(this,
          node,
  ...
  bindEvent(ele) {
    ele.addEventListener('click', e => {
      ...
      else if(target.nodeName === 'SPAN' && hasClass(target, 'tx-switcher')) {
        e.stopPropagation();
        this.onSwitcherClick(target);
      }
      else{ 
          if(this.options.templates){
            let isFleet = hasClass(target, 'tx-fleet-ribbon');
            let isLeaf = hasClass(target, 'tx-leaf-card');
            let fleetChildren = target.closest(".tx-fleet-ribbon");
            let leafChildren = target.closest(".tx-leaf-card");

            if(fleetChildren || isFleet) { // CLICK FLEET
                e.stopPropagation();
                let myTarget = (isFleet)? target : fleetChildren;
                //console.log("flota clickeada", myTarget, target);
                let selection = myTarget.closest("li.tx-node");
                
                updateNodeSelectionStyles(myTarget, this.container+" .tx-fleet-ribbon");
                this.fleetNodeSelected = selection;
                ...
                if(this.leafNodeSelected && this.fleetNodeSelected.contains(this.leafNodeSelected)){
                    ...
                    updateNodeSelectionStyles(null, this.container+" .tx-leaf-card");
                }
                
                this.elem.dispatchEvent(new CustomEvent("fleet-node-selected", {
                    detail: {
                        fleetNodeSelected: selection.nodeId,
                        leafNodeSelected: (selectionLeaf)? selectionLeaf.nodeId : null
                    }
                }));
            }
            else if(leafChildren || isLeaf) { // CLICK LEAF
                e.stopPropagation();
                let myTarget = (isLeaf)? target : leafChildren;
                //console.log("recurso clickeado", myTarget, target);
                let selection = myTarget.closest("li.tx-node");
                let newSelection = myTarget.closest("li.tx-fleet");
                
                let selectionFleet; 
                if(this.fleetNodeSelected && newSelection == this.fleetNodeSelected) { //this.fleetNodeSelected.contains(selection)){
                    selectionFleet = this.fleetNodeSelected;
                }else{
                    selectionFleet = selection.parentElement.parentElement;
                    updateNodeSelectionStyles(selectionFleet.querySelector(".tx-fleet-ribbon"), this.container+" .tx-fleet-ribbon");
                    this.fleetNodeSelected = selectionFleet;
                }
                
                updateNodeSelectionStyles(myTarget, this.container + " .tx-leaf-card");
                this.leafNodeSelected = selection;
                
                this.elem.dispatchEvent(new CustomEvent("leaf-node-selected", {
                    detail: {
                        fleetNodeSelected: selectionFleet.nodeId,
                        leafNodeSelected: selection.nodeId,
                        target: e.target,
                    }
                }));
            }
  ...
  emptyNodesCheckStatus() {
    ...
    Object.values(this.willUpdateNodesById)
      .forEach(node => {
  ...
  collapseAll() {
    let switchers = document.querySelectorAll(this.container+' .tx-node__open > .tx-fleet-header > .tx-switcher');
    console.log(switchers.length);
    switchers.forEach(sw => this.onSwitcherEvent(sw, true));
  }
  
  expandAll() {
    let switchers = document.querySelectorAll(this.container+' .tx-node__close> .tx-fleet-header > .tx-switcher');
    console.log(switchers.length);
  ...
  static createLiEle(node, closed) {
    ...
    if (closed){ 
        li.classList.add('tx-node__close');
    }
    else{
        li.classList.add('tx-node__open');
    }
    if (node.isFleet){ 
        li.classList.add('tx-fleet');
    }
    else{
        li.classList.add('tx-leaf');
    }
    
    if (node.isFleet) {
        const switcher = document.createElement('span');
        if(node.children && node.children.length){
            switcher.classList.add('tx-switcher');
        }
        else{
            switcher.classList.add('tx-void');
        }
    ...
    if(this.options.templates && "template" in node){
        ...
        if (node.isFleet) {
            div.classList.add("tx-fleet-ribbon");
            header.classList.add('tx-fleet-header');
        }
        else{
            div.classList.add("tx-leaf-card");
            header.classList.add('tx-leaf-header');
        }
