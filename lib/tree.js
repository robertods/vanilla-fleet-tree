class Tree {
  constructor(container, options) {
    ...
    this.elem.classList.add('tx-list-tree')
  ...
  handleClick(e) {
    const { target } = e;
    if (target.nodeName === "SPAN" && (treeUtil.hasClass(target, "tx-checkbox") || treeUtil.hasClass(target, "tx-label"))) {
      e.stopPropagation();
      this.onItemClick(target.parentNode.parentNode.nodeId);
      return;
    }
    
    if (target.nodeName === "SPAN" && treeUtil.hasClass(target, "tx-switcher")) {
      e.stopPropagation();
    ...
    let isFleet = treeUtil.hasClass(target, "tx-folder-ribbon");
    let isLeaf = treeUtil.hasClass(target, "tx-leaf-card");
    ...
    if (fleetChildren || isFleet) {  // CLICK FOLDER
      ...
      treeUtil.updateNodeSelectionStyles(
        myTarget,
    ...
    if (leafChildren || isLeaf) {  // CLICK LEAF
      ...
      if (this.fleetNodeSelected && newSelection == this.fleetNodeSelected) {
        ...
        treeUtil.updateNodeSelectionStyles(selectionFleet.querySelector(".tx-folder-ribbon"), this.container + " .tx-folder-ribbon");
        this.fleetNodeSelected = selectionFleet;
      }

      treeUtil.updateNodeSelectionStyles(myTarget, this.container + " .tx-leaf-card");
  ...
  onSwitcherClick(target) {
    ...
    if (this.options.animation) {
      ...
      if (treeUtil.hasClass(liEle, "tx-node__close")) {
        treeUtil.animation(150, {
          enter() {
            ele.style.height = 0;
            ele.style.opacity = 0;
          },
  ...
  filterNodes(e) {
    ...
    queries.forEach(q => {
      q = q.trim()
      let search1 = this.filterForFolder(q)
      let search2 = this.filterForLeaf(q)
      matchedFolders = treeUtil.uniques([...matchedFolders, ...search1[1], ...search2[1]])
      matchedLeafs = treeUtil.uniques([...matchedLeafs, ...search1[0], ...search2[0]])
    })
    
    this.elem.querySelectorAll('.tx-node').forEach(n => n.classList.add("search-hide"))
    matchedFolders.forEach(n => n.classList.remove("search-hide"))
    matchedLeafs.forEach(n => n.classList.remove("search-hide"))
  }

  filterForFolder(q) {
    // nodes
    const searched = treeUtil.contains(this.elem, '.tx-folder-ribbon > .search-clue', q)
    const nodes = searched.map(el => el.closest('.tx-folder'))
    // parents
    const folders1 = nodes.reduce((total,el) => [ ...total, ...treeUtil.getAncestors(el, '.tx-folder')], [])
    // children
    const allChildren = nodes.reduce((total,n) => [ ...total, ...n.querySelectorAll('.tx-leaf')], [])
    const children = treeUtil.uniques(allChildren)
    // children parents
    const folders2 = children.reduce((total,el) => [ ...total, ...treeUtil.getAncestors(el, '.tx-folder')], [])
    const folders = treeUtil.uniques([...nodes, ...folders1, ...folders2])
    return [children, folders]
  }
  filterForLeaf(q) {
    // nodes
    const searched = treeUtil.contains(this.elem, '.tx-leaf-card > .search-clue', q)
    const nodes = searched.map(el => el.closest('.tx-leaf'))
    // parents
    const allFolders = nodes.reduce((total,el) => [ ...total, ...treeUtil.getAncestors(el, '.tx-folder')], [])
    const folders = treeUtil.uniques(allFolders)
  ...
  set values(values) {
    return this.setValues(treeUtil.uniques(values));
  }
  ...
  set disables(values) {
    return this.setDisables(treeUtil.uniques(values));
  }
  ...
  static parseTreeData(data) {
    const treeNodes = treeUtil.deepClone(data);
    const nodesById = {};
    const leafNodesById = {};
    const values = [];
    const disables = [];
  ...
  static createLiEle(node, closed) {
    ...
    if (this.options.templates && "template" in node) {
      ...
      const templateFunction = this.options.templates[node.template] || this.options.templates['FLEET'];
      div.innerHTML = templateFunction(node);
...
const treeUtil = {
  ...
  updateNodeSelectionStyles(myTarget, selector) {
