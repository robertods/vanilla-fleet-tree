class Tree {
  ...
  filterNodes(e) {
    ...
    queries.forEach(q => {
      ...
      matchedFolders = uniques([...matchedFolders, ...search1[1], ...search2[1]])
      matchedLeafs = uniques([...matchedLeafs, ...search1[0], ...search2[0]])
    })
  ...
  filterForFolder(q) {
    // nodes
    const searched = contains(this.elem, '.tx-folder-ribbon > .searchClue', q)
    const nodes = searched.map(el => el.closest('.tx-folder'))
    // parents
    const folders1 = nodes.reduce((total,el) => [ ...total, ...getAncestors(el, '.tx-folder')], [])
    // children
    const allChildren = nodes.reduce((total,n) => [ ...total, ...n.querySelectorAll('.tx-leaf')], [])
    const children = uniques(allChildren)
    // children parents
    const folders2 = children.reduce((total,el) => [ ...total, ...getAncestors(el, '.tx-folder')], [])
    const folders = uniques([...nodes, ...folders1, ...folders2])
    return [children, folders]
  }
  filterForLeaf(q) {
    // nodes
    const searched = contains(this.elem, '.tx-leaf-card > .searchClue', q)
    const nodes = searched.map(el => el.closest('.tx-leaf'))
    // parents
    const allFolders = nodes.reduce((total,el) => [ ...total, ...getAncestors(el, '.tx-folder')], [])
    const folders = uniques(allFolders)
